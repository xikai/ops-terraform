terraform {
  ...
  #   region              = "cn-shenzhen"
  #   tablestore_endpoint = "https://company-terraform.cn-shenzhen.ots.aliyuncs.com"
  #   tablestore_table    = "statelock"
  # }
    backend "s3" {
    dynamodb_table = "company-terraform-state-lock"
    bucket         = "company-terraform-remote-state"
...
module "ecs" {
  ...
  user_data = templatefile("../../modules/alicloud/ecs/middleware_ansible.tftpl", {
    ...
    oss_region     = "company-packages.oss-cn-shanghai"
    mount_data_dir = "/data"
  })
}
